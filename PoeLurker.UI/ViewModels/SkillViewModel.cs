    ...
    public class SkillViewModel : Caliburn.Micro.PropertyChangedBase
    {
        ...
        public void OnClick()
        {
            if (this._justChecked)
            {
                ...
                if (this.Selected)
                {
                    this._eventAggregator.PublishOnUIThreadAsync(new SkillMessage() { Skill = this._skill });
                }
            ...
            this.Selected = false;
            this._eventAggregator.PublishOnUIThreadAsync(new SkillMessage() { Skill = this._skill, Delete = true });
        }
